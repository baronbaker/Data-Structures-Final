...
int FindMinPath(struct AVLTree *tree, TYPE *path)
{
  ...
  while (current->left != NULL || current->right != NULL) {
    ...
    if (current->left == NULL) {
      ...
      int diff_left = abs(current->val - current->left->val);
      int diff_right = abs(current->val - current->right->val);
      current = (diff_left <= diff_right) ? current->left : current->right;
    }
  }
...
void printBreadthFirstTree(struct AVLTree *tree)
{
    ...
    /* FIX ME */
  struct AVLnode **queue = malloc(tree->cnt * sizeof(struct AVLnode *)); /* Assuming maximum 10000 nodes */
  int front, rear;
